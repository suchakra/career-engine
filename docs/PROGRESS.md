...
| Phase 7 — Job Discovery web surface | 🟡 building (7A done) | Bring the CLI-only discovery loop into the web app as a "Jobs" view ([GROOMING.md Phase 7](GROOMING.md)). **7A DONE** (PR #38, **contract v2.8.0**): persist the rubric — `UserWorkspace.discovery_preferences: SessionPreferences` (additive) + `web/preferences_store.py`. **Pending: 7B** (Jobs view: runner + view-model + renderer + nav + wiring) → **7C** ("Tailor to this job"). Discovery stays CLI-only until 7B/7C land; ARCHITECTURE §15 web-surface reconciliation happens at the Phase 7 exit gate. |
