...
Last updated: **2026-07-07** — *Phase 10 building API-first; **slice 10.1 merged (PR #63)**. PR #59 recorded the Streamlit→Next.js+FastAPI decision (ARCHITECTURE §16) + build tickets 10.1-10.7; PR #60 resolved the 10.1 auth shape (Firebase bearer, AD-16.4) + UI mockup + Phase 11 roadmap; PR #61 added the context-management strategy; PR #62 added the component inventory + AD-16.8 (TanStack Query data layer) + AD-16.9 (frontend test stack: Vitest/RTL/MSW/Playwright) + pinned Node 22 in the devcontainer; PR #63 shipped 10.1 — the FastAPI app + single Firebase-bearer auth boundary (`api/`, `GET /api/health` + `GET /api/me`). master @ `6d7b163`, contract v2.8.0, 707 tests (1 skipped). Next: slice 10.2 (read APIs), then 10.3→10.7, one PR each.*
...
| Phase 10 — Migrate to Next.js + FastAPI | 🟡 building | ✅ 10.0 decision recorded ([ARCHITECTURE.md §16](ARCHITECTURE.md), PR #59); AD-16.8/16.9 client-data + toolchain (PR #62); **✅ 10.1 FastAPI skeleton + Firebase-bearer auth boundary merged (PR #63)**. 10.2-10.7 are ✅ Ready build tickets in [GROOMING.md §Phase 10](GROOMING.md). API-first; building one slice per PR — **10.2 (read APIs) next**. |
