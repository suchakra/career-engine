...
Last updated: **2026-07-05** — *Phase 6 (two-agent A2A job discovery) built on `feat/discovery-a2a`, contract **v2.5.0**, PR #30 (Sonnet PASS + Copilot addressed); see the Phase 6 row + [ARCHITECTURE.md §15](ARCHITECTURE.md). Prior: Phase 2 COMPLETE; contract **v2.3.0** (tag `contract-v2.3.0`). **Phase 3 started:** eval harness merged via **PR #1** (squash, **389 tests**) + `wait-for-pr-review` skill. **Copilot budget reset → PR-based workflow** (branch → Sonnet review → PR → Copilot review via `wait-for-pr-review` → squash-merge). **Ordered queue:** (1) security review ✅ (PR #2) → (2) monitoring/logging ✅ (PR #3, 405 tests) → (3) CoT tuning ✅ (PR #4, **contract v2.3.0**, tag `contract-v2.3.0`, 409 tests) → (4) Phase 2 deferred wiring ✅ (PR #5) → (5) capstone runbook dry-run ✅ (PR #6, 424 tests). **Phase 3 queue COMPLETE.***
...
| Phase 7 — Job Discovery web surface | ✅ COMPLETE | Discovery is now a **web product feature** ([ARCHITECTURE §15.6](ARCHITECTURE.md)), not CLI-only. **7A** (PR #38, **contract v2.8.0**): persist the rubric — `UserWorkspace.discovery_preferences` + `web/preferences_store.py`. **7B** (PR #39): "Jobs" nav view — `web/jobs.py` (view-model + renderer), `web/jobs_runner.py` (BYOK loop via `run_async`, `LedgerStore.list_accepted` for prior matches), `_render_jobs` (preferences form → **Find jobs** → ranked ✅/🟡 with rationale). **7C**: per-job "Tailor résumé to this" hands the JD to the Tailor (`job_tailor_index`). Engine unchanged; grill → Jobs → tailor in the UI. LIVE web-path smoke verified. **Post-Phase-7 HITL:** a per-job "Not interested" dismisses a company (persisted to the ledger via `add_rejected_company`; future runs hard-reject it). |
