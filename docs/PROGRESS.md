...
Phase 3 remaining (post-queue): the ordered queue continues with **(4) Phase 2 deferred wiring** (Streamlit discovery-session load for the meter; sweep Cloud Run HTTP endpoint + IdP token exchange; add `terraform` to the devcontainer) → **(5) capstone runbook dry-run**.
