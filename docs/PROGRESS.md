...
| Phase 5 — Tailoring & résumé quality | 🟡 (5A/5B/Contact done) | **5A DONE** (PR #29): real ATS-safe résumé grouped by role + education + JD-aligned skills; `web/resume_render.py` PDF/DOCX/MD. **5B DONE** (PR #31): save a tailored résumé as a tracked `Application` (dashboard + 14-day sweep; no contract change). **Persist Contact DONE** (PR #32, **contract v2.6.0**): `UserProfile` on `UserWorkspace` + `web/profile_store.py`; the Tailor contact header pre-fills + persists. Remaining: **5C** one renderer for master+tailored, **4E** highlight, pre-GA **/security-review**. See [REFINED_PROJECT_PLAN.md](REFINED_PROJECT_PLAN.md) Phase 5. |
