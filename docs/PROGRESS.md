...
| Phase 5 — Tailoring & résumé quality | 🟡 | **5A DONE** (PR #29): real ATS-safe résumé — `web/resume_builder.py` groups validated stories under their `Entry` (experience by role) + education + JD-aligned skills + tailored summary; `web/resume_render.py` PDF/DOCX/MD; no "why it fits" in the doc. Removed the flat tailor/exporter. Remaining: **persist Contact** (parser/profile → +minor), **5B** save-as-tracked-application, **5C** one renderer for master+tailored, 4E highlight, pre-GA /security-review. See [REFINED_PROJECT_PLAN.md](REFINED_PROJECT_PLAN.md) Phase 5. |
...
| Phase 6 — Two-agent (A2A) job discovery (capstone) | 🟡 build ✅ / review+package pending | **All 5 build steps done on branch `feat/discovery-a2a` (contract v2.5.0, 559 tests).** Ontology (`JobOpportunity`/`EvaluationDiff`/…) → real FastMCP server `discovery/mcp_server.py` over a live key-free source → stateless `Scout` (MCP client) → stateful `PrimaryAgent` (deterministic hard-reject gate + injectable agentic evaluator + bounded MAX_ITERATIONS=3 loop → `EvaluationDiff`) → `career-engine discover` + idempotent `LedgerStore` → Tailor reuse. LIVE end-to-end run verified. **Pending:** Sonnet+Copilot review, PR/merge/tag, and PACKAGING (video/writeup/README/diagram). Spec [ARCHITECTURE.md §15](ARCHITECTURE.md). |
