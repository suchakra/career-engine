...
from web.grill_labels import _effective_frontier_label, _frontier_label
...
def _try_resume(user_id: str) -> None:
    ...
    ss["grill_entry_label"] = _effective_frontier_label(state)
    ss["grill_started"] = True
...
def render_grill(*, user_id: str) -> None:
    ...
    if ss.get("grill_checkpoint"):
        st.info(
            "� You've reached a checkpoint — stories completed so far are visible in "
            "**Portfolio**."
        )
